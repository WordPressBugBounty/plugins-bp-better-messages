...
* Fixed My profile still being offered at the foot of the settings screen on a phone when the Appearance switch for it was turned off
* Fixed the last conversation in the list staying on screen after it was erased, deleted or left, until the page was reloaded
* Fixed a chat room's online users list taking its width out of the conversation during a call, which left messages wrapping at about one character a line
* Fixed the Login button on a chat room's sign-in prompt taking its label colour from the theme, which left it unreadable on the accent fill in some colour schemes
* Fixed the Indigo palette's dark mode putting white on its accent, where the other palettes already use dark ink
* Fixed the user settings buttons — "Restore encryption keys", "Free up space" and "Clear local data" — arriving in dark mode as outlines with nothing legible inside them, and "Clear local data" not reading as the destructive action it is
* A microphone or camera that will not open now says why — not allowed yet, missing, or already in use by another application — and points at the fix it needs
* Security improvement: a sent message now carries only the metadata the messenger is meant to send, with everything else written on the server — developers sending custom metadata can allow their keys through the new `better_messages_client_message_meta_keys` filter
* Security improvement: a display name can no longer carry an element into the messenger — names now pass an allowlist on the server and in the browser, which badges, icons and the links around them pass untouched, and a tag it does not carry can be added through the new `better_messages_display_name_allowed_html` filter in [PHP](https://www.better-messages.com/hooks/php-filters/#better_messages_display_name_allowed_html) and [JavaScript](https://www.better-messages.com/hooks/js-filters/#better_messages_display_name_allowed_html)
* The "Chatting as …" bar above the message field, with its login and register links, is back on every screen a guest can be on — mini chats included, which had never carried it
* A link to a chat room opens the Chat Rooms tab again, rather than leaving the list on Conversations
* A role that may not start conversations, guests included, no longer gets a "Start a new conversation" button on the screen beside the conversations list
* Every link inside a message opens in a new tab again, markdown links and links written as HTML included
* Fixed "Pointing tail toward the avatar" pointing away from it, outlined messages being unable to carry the tail at all, and an outlined message losing its own edge while it was clicked
* Fixed the reason a conversation could not be started never reaching the member who tried, or staying in the address bar and coming back as a stray error on the next screen — most visible on the interval a member must wait between starting conversations
* The reply, favorite and more buttons are back beside messages in mini chat windows, now as a setting — **Appearance → Mini chat windows → Message actions in mini chats**, on by default as in 2.x
* Opening an image no longer hides the ways to answer it — the viewer's toolbar now carries Reply, the favorite star and the whole message menu
* Whether a tap on a message reveals its buttons is now yours to choose — **Appearance → Conversation → Messages → Tap a message to show its actions**, on by default as it has been since 2.x
* Fixed the buttons beside a message being clipped at the edge of the conversation, which on a phone cut the last one in half
* Fixed "Copy message text" being offered on a message with no text to copy, such as a voice message, a sticker, a GIF or attachments sent without a caption
* Notification cards are yours to colour — **Appearance → Colors → Notifications** now carries their background, text and border in both schemes, and a choice of Inverted as before or Match the messenger
* Fixed typing @ in a presence-based chat room offering no names — the list is now built from whoever is in the room and whoever has spoken in it
* Fixed the Appearance preview filling the debug log with WordPress deprecation notices every time the settings page was opened
* Fixed an email notification from a chat room arriving with `&#038;` in its subject line in place of the `&` in the room's name, and the same for an apostrophe, a quotation mark, a dash and an ellipsis — a room named before this is corrected as the email goes out, so nothing has to be re-saved
* Reliability improvements for one-to-one calls — a call whose connection fails now recovers, or moves to the room, in seconds rather than tens of seconds, and the iOS and Android apps need a new build to get them
* Erasing a conversation removes it from your own list straight away, rather than leaving a row that opens to "the conversation does not exist" — leaving one, pinning, muting and editing a message all answer at once in the same way
* "Clear messages" now clears the whole conversation, instead of removing the first fifty messages and reporting success with the rest still there
* A conversation that no longer exists, or that you can no longer open, is cleared from the messenger for guests too, and one left behind by a missed realtime message is noticed again after a reload
* The time on a hover card keeps counting while the card is open, in the narrow conversations rail and on the chat heads alike
* Other minor bugfixes and improvements
